...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.45\linewidth}
        ...
        \label{fig39.1-2}
    \end{subfigure}
    \bicaption{基于迈克尔逊干涉原理设计的两版自相干仪光路结构。(a) 第一版自相干仪；(b) 第二版自相干仪}{Two versions of the autocorrelator optical layout designed based on the Michelson interferometer principle. (a) Mark 1; (b) Mark 2}
    \label{fig39.1}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \bicaption{双椭圆扫描系统功能框图、稳定性分析及扫描速率关系。(a) 双椭圆级联匀速扫描系统功能框图；(b) 不同 $e_2$ 下扫描速度放大倍率 $k$ 的稳定性分析；(c) 双椭圆扫描入射与出射光几何关系(子图A)及扫描速率关系(子图B)}{Functional block diagram, stability analysis, and scan-velocity relationship of the double-ellipse scanning system. (a) Functional block diagram of the cascaded uniform-speed double-ellipse scanner; (b) stability of the scan-velocity magnification factor $k$ under different $e_2$ values; (c) Geometric relationship between incident and outgoing light in double elliptical scanning (subgraph A) and scanning rate relationship (subgraph B)}
    \label{fig:fig49_52_62}
\end{figure}


经过参数优化与结构校正后，双椭圆系统在全视场范围内表现出较高的扫描均匀性。最终结果如图\ref{fig:fig62}(B) 所示，在目标扫描范围内，出射角对时间的变化关系近似线性，其最大偏差仅为 $3.65\%$，均方根误差(RMSE)低至 $0.19$。这表明所提出的双椭圆级联系统已能够较好地逼近理想匀速扫描模型。相比传统扫描结构，该方法不仅改善了几何光程分布，还显著提升了扫描动态特性的一致性。
