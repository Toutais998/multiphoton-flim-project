...
\caption{40MHz激发频率下不同扫描速度和数字化像素数对应的单像素时间以及单像素包含脉冲数关系}
\label{Table2.1}
\end{table}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{Fig/Fig03.pdf}
    \caption{数字化采样对分辨率的影响}
    \label{fig03}
\end{figure}

》 这张图考虑一下是否需要添加引用

如图\ref{fig03}在子图a中的原始高分辨率图像经过欠采样的像素化后得到子图b中的图像，该过程会降低图像分辨率。子图c表示将连续的模拟信号数字化采样为离散的数字信号过程采样像素大小对数据的影响
...
在激发光束充分填充物镜后孔径(overfilling)且系统像差较小的近似条件下，焦点附近的光强分布可表示为时间项与空间项的乘积形式，即
\begin{equation}
...
其中 $NA$ 为物镜数值孔径，$\lambda$ 为激发波长，$P(t)$ 为入射光功率。需要说明的是，式~(\ref{eq:I0_scaling}) 仅表示峰值光强的标度关系，其具体系数与光束类型、后孔径填充程度以及像差情况有关，因此更适合写成比例关系，而不宜在缺少完整推导时直接写成严格等式。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{Fig/Fig07.png}
    \caption{点扩散函数及其宽度表征参数示意图}
    \label{fig07}
\end{figure}

》这展图是否需要添加引用，并且加入中文标注

图~\ref{fig07}(a)\cite{RN1177} 给出了线性激发与双光子激发条件下焦点附近能量分布的对比。左侧两幅图分别表示线性强度分布 $I(x,y,z)$ 在轴向和横向截面上的形态；右侧两幅图表示对应的平方分布 $I^2(x,y,z)$。可以看出，非线性激发后，主瓣范围明显缩小，尤其是在轴向方向上的压缩更加明显，这意味着只有更靠近焦点中心的区域能够有效产生荧光信号。

图~\ref{fig07}(b) 则说明了描述 PSF 宽度的几种常用指标：FWHM 表示强度下降到峰值一半时对应的全宽，$\omega$ 通常表示高斯分布中 $1/e^2$ 强度位置对应的特征半径。对于高斯函数，这些宽度定义虽然数值不同，但都可用来表征聚焦光斑或 PSF 的空间尺度。
...
因此，在染料种类与浓度固定、探测链路基本不变的条件下，双光子信号近似与平均功率平方成正比，而三光子信号近似与平均功率立方成正比。而平均功率\ref{eq:peak_power_scaling}分子受限于组织或样本的热损伤、荧光的热漂白等影响无法无限增加，因此分母$f\tau$对多光子效应成像效果贡献为优化的高优先级，具体推导请查看第\ref{4.1}节。与此同时，更高的有效数值孔径通常会带来更强的聚焦能力和更小的有效激发体积，从而提高局域激发效率。但这种依赖关系通常通过式~(\ref{eq:I0_scaling}) 和式~(\ref{eq:m_signal_scaling}) 间接体现，而不宜在缺少统一推导前直接写成简单的 $NA^2$ 或更高次幂形式。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{Fig/Fig08.pdf}
    \caption{点扫描中四种代表性扫描轨迹}
    \label{fig08}
\end{figure}

》　这张图说明你的产生代码，不需要添加引用

图\ref{fig08}-a: 栅格扫描 (Raster Scan)，通过快慢轴锯齿波驱动，实现均匀的全画幅覆盖，是常规共聚焦及荧光寿命成像 (FLIM) 的标准模式；

图\ref{fig08}-b: 李萨如扫描 (Lissajous Scan)，利用两个不同频率的正弦波驱动，可在极短时间内实现中心区域的高频采样，适用于超高帧率成像如血流动态或快速钙信号监测 ；

图\ref{fig08}-c: 螺旋扫描 (Spiral Scan)，通过振幅随时间线性增加的正弦和余弦波驱动，常用于荧光相关光谱 (FCS) 等中心区域的高频采样追踪 ；

图\ref{fig08}-d: 随机/矢量扫描 (Random/Vector Scan)，根据预设目标区域 (Target ROI) 任意定义电压-位置关系，能够最大化跳过无效暗区，常用于光遗传学刺激或靶向光漂白等精准操控场景 。不同扫描模式在帧率限制因素及典型应用场景上各有侧重。
...
除机械扫描镜外，声光偏转器(AOD)与电光偏转器(EOD)等非机械扫描可实现更高带宽与随机访问能力，适合高速 ROI/随机点扫描与三维快速跳转等应用；但其扫描角、色散补偿、系统复杂度与功率利用率等因素需要在多光子系统中综合评估。对于特定需求(例如大范围拼接、长时程稳定成像)，载物台扫描或平台辅助扫描仍具有工程价值。
...
对于本文采用的 Olympus 25$\times$/1.05 NA 物镜，由式~(\ref{eq:f0_olympus25x}) 得
\begin{equation}
...
其中 $\mathrm{NA}_{0}$ 为物镜标称数值孔径。该式表明：在欠填充条件下，系统实际参与聚焦的激发 NA 将随 BFP 处束径减小而下降，从而导致激发点扩散函数(PSF)展宽，表现为横向与轴向分辨率下降。
...
系统类型 & 无穷远校正 \\ \hline
\end{tabular}
\caption{本文实验系统所用 Olympus 25$\times$/1.05 NA 水浸物镜的关键参数}
\label{tab:olympus_obj}
\end{table}

》　这个表格，表头都要放在前面去，不能放在表后面
...
除商业物镜选型外，本文还尝试在空气物镜基础上设计一套 10$\times$/0.8 NA 物镜，用于分析较高数值孔径与较大扫描视场条件下的光学设计可行性。该设计在 Zemax 中完成优化，并将原始玻璃材料替换为 CDGM 玻璃，以提高后续加工实现的可行性。图~\ref{fig09} 给出了该物镜的光学设计结果，包括A: 结构布局、B: modulation transfer function (MTF) 曲线以及C: 像差分析结果。结果表明，该设计在目标视场范围内具有较好的成像性能，可为后续扫描显微系统中的物镜设计提供参考。


\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{Fig/Fig09.png}
    \caption{自研 10$\times$/0.8 空气物镜的 Zemax 光学仿真}
    \label{fig09}
\end{figure}
