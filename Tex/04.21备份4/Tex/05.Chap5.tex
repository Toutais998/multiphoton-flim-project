...
在完成系统搭建、同步方式建立以及数据处理流程开发后，本文进一步对荧光寿命成像结果进行了实验验证。图~\ref{fig26-27}(\subref{fig26}) 给出了基于商用 TCSPC 方法和基于单光子计数方法获得的 FLIM 图像结果。
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.49\textwidth}
    ...
    \label{fig27}
  \end{subfigure}
  \bicaption{反射式、TCSPC 与光子计数方式下的 FLIM 成像结果。(a) 基于反射式共聚焦、商用 TCSPC 以及单光子计数重分配流程获得的典型成像结果；(b) 针对a图优化激发功率的荧光寿命成像对比结果}{FLIM results obtained using reflective imaging, TCSPC, and photon-counting approaches. (a) Representative images acquired by reflective confocal imaging, a commercial TCSPC board, and the single-photon-counting reassignment workflow; (b) Optimization of excitation power for figure a}
  \label{fig26-27}
\end{figure}
...
从结果上看，两种不同方法均能够恢复出样品的寿命分布信息，但初始结果中仍存在由成像功率和系统分辨率设置不当带来的局部过曝与光子堆积问题。为进一步提高寿命拟合质量，本文根据初步结果对成像功率和系统参数进行了优化，并重新采集得到图~\ref{fig26-27}(\subref{fig27}) 所示结果。
...
在单光子探测系统中，系统的总死区时间不仅受限于单光子雪崩二极管(SPAD)自身的物理特性，还受到数据采集卡(DAQ)硬件响应机制的制约。图~\ref{fig29} 从前端探测器响应、标准采集流程、采集卡重置限制以及 long record 优化策略四个层面，给出了 SPAD 探测器与数据采集卡死区时间的时序分析示意。

其中，前两幅子图主要说明系统死区的来源与标准采集流程，后两幅子图则进一步展示采集卡在高计数率条件下的漏触发问题，以及通过 long record 策略提升有效采集效率的实现思路。
...
为了降低固定重置时间对系统整体探测效率的影响，本文采用了增加单次记录长度(long record)的优化策略，如图~\ref{fig29}(\subref{fig29d}) 所示。通过延长单次采集记录窗口，可以降低固定重置时间在总采集周期中的占比，从而提高系统的有效采集效率。系统的有效采集效率可定义为
=======
