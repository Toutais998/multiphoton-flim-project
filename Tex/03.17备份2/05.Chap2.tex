...
\begin{figure}[htbp]
    ...
    \bicaption{数字化采样对分辨率的影响}{Impact of digital sampling on spatial resolution}
    \label{fig03}
\end{figure}

如图~\ref{fig03}所示，子图(a)表示原始高分辨率连续图像，子图(b)表示该图像在较大采样间隔下经像素化后的结果。随着采样像素尺寸增大，高频细节会被平均化甚至发生混叠，最终表现为边缘模糊、细节丢失以及结构失真。子图(c)进一步示意了连续模拟信号被数字化为离散采样点的过程，说明数字化采样不仅决定图像的数据表示形式，也直接约束系统能够有效保留的空间频率范围。因此，在扫描显微成像中，数字化分辨率必须与光学分辨率相匹配，才能充分发挥系统的成像能力。
...
\begin{figure}[htbp]
    ...
    \bicaption{点扩散函数及其宽度表征参数示意图}{Schematic of the point spread function and its width metrics}
    \label{fig07}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \bicaption{自研 10$\times$/0.8 空气物镜的 Zemax 光学仿真}{Zemax optical simulation of a self-designed 10$\times$/0.8 air objective}
    \label{fig09}
\end{figure}
